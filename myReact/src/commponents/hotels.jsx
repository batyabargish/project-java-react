...
export const Hotels = () => {
  ...
  return (
    ...
    >
      ...
      <Typography variant="h3" gutterBottom sx={{ color: 'rgba(233, 178, 68, 0.8)',fontWeight: 'bold',  
            ...
            r.sug === "Hotel" && (
              <Card key={r.id} sx={{ maxWidth: 345, margin: '20px', borderRadius: 2, boxShadow: 3 ,direction: 'rtl', backgroundColor: 'rgba(255, 255, 255, 0.9)' ,height:"250px" ,width:"300px"}}>
                ...
                <CardActions>
                  ...
                  <Link to={`/hotels/${r.id}`} style={{ textDecoration: 'none' }}>
  <Button
    size="small"
    variant="contained"
    sx={{
      backgroundColor:'rgba(238, 203, 133, 0.8)', // צבע רקע כתום
      color: 'white', // צבע טקסט לבן
      '&:hover': {
        backgroundColor: 'rgba(233, 178, 68, 0.8)', // צבע רקע ב-hover (כשהכפתור בעכבר)
      },
    }}
  >
    פרטים נוספים על המלון
  </Button>
</Link>
                  {cUser && cUser.admin && (
...
export const HotelDetails = () => {
  ...
  const handleRedirect = () => {
    if (recreation && recreation.link) {
      window.open(recreation.link, '_blank'); 
    }
  };
    ...
    >
      ...
      >
       ...
       sx={{
        ...
        borderColor: 'rgba(202, 149, 44, 0.8)', // מסגרת הכפתור בצבע הכיתוב
        '&:hover': {
          borderColor: 'rgba(202, 149, 44, 0.8)', // שמירה על הצבע בעת ריחוף
          backgroundColor: 'rgba(202, 149, 44, 0.1)', // אפשר להוסיף שינוי צבע רקע בהHover
        }
      }}
      variant="outlined"
      color="inherit"
